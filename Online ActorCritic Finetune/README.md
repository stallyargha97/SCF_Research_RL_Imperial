...
- **Critic ← CQL** — the "first critic" is obtained by loading the trained CQL policy
  (`cql_cirl_actor_anti_windup_best.pt`), rolling it out on the data, and fitting the critic to
...
├── policies/   warm-start inputs: bc_cirl_..._best.pt (actor), cql_cirl_actor_anti_windup_best.pt (critic src)
