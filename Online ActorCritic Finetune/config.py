...
# Old narrow [Kp, Ki, Kw] gain box (margin |Kw*Ki| = 0.06), kept for USE_OLD_BOX-style
# comparisons/notebooks — do not remove.
OLD_GAIN_LOW  = np.array([-6.0, -0.100, -0.60], dtype=np.float32)
OLD_GAIN_HIGH = np.array([-0.05, -0.0001, 0.10], dtype=np.float32)

# box0.8 gain box (margin |Kw*Ki| = 0.8): best robustness/accuracy trade-off from the
# box0.4/0.8/1.2 sweep — box0.4 diverged 10/10 on the random-stream screen, box1.2 had
# repeated catastrophic training collapses despite a similar screen score. New default.
GAIN_LOW  = np.array([-14.7192, -1.51687, -0.52734], dtype=np.float32)
